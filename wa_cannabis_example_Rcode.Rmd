...
*Hash is the New Encore*: Bayesian Time Series Forecast using Prophet
```{r}

# Step 1: Identify top 2 selling product categories
top_categories <- wa_canna %>%
  count(inventory_type_category, wt = retail_sales_amt, sort = TRUE) %>%
  slice_head(n = 2) %>%
  pull(inventory_type_category)

# Step 2: Filter time series for just those categories
category_ts_bayes <- category_ts %>%
  filter(inventory_type_category %in% top_categories)

# Step 3: Clean for valid time series modeling (at least 2 non-zero entries)
category_ts_bayes_clean <- category_ts_bayes %>%
  group_by(inventory_type_category) %>%
  filter(sum(!is.na(retail_sales_amt) & retail_sales_amt > 0) >= 2) %>%
  ungroup()

# Step 4: Fit NNETAR model (Bayesian-like neural net autoregression)
category_bayes_model <- category_ts_bayes_clean %>%
  model(
    bayes = NNETAR(retail_sales_amt)
  )

# Step 5: Forecast next 6 months with confidence intervals
category_forecasts <- forecast(category_bayes_model, h = "6 months", level = c(80, 95))

# Step 6: Check if confidence levels exist in forecast
if (".level" %in% colnames(category_forecasts)) {
  
  # Step 6a: Convert forecast to wider format for plotting ribbons
  ...
  # Step 9: Fallback for models that do NOT support CIs (e.g., NNETAR in some settings)
...
Let me know if you'd like the same treatment for strain-level decomposition and forecasting
